...
from wgan import generator_loss, discriminator_loss
...
load = True
if load:
    generator = torch.load("../../outputs/wgan/generator.pt")
    discriminator = torch.load("../../outputs/wgan/discriminator.pt")
else:
...
def train_wgan_batches(epoch, G, D, dataloader, batch_size, train=True, save=True):
    ...
    for i, (imgs, tgts) in enumerate(dataloader):
        ...
        g_loss = generator_loss(D(gen_imgs))
        if train:
            g_loss.backward()
            g_optimizer.step()
        ...
        if iteration % sample_interval == 0:
            ...
            save_image(imgs.data[:4], "../../outputs/wgan/%d_" % iteration + pathval + "_input.png", nrow=2)
            save_image(gen_imgs.data[:4], "../../outputs/wgan/%d_" % iteration + pathval + "_output.png", nrow=2)
            save_image(tgts.data[:4], "../../outputs/wgan/%d_" % iteration + pathval + "_target.png", nrow=2)
        if save and (iteration % save_interval == 0): 
            torch.save(generator, "../../outputs/wgan/generator.pt")
            torch.save(discriminator, "../../outputs/wgan/discriminator.pt")
...
def train_wgan(generator, discriminator, save=True):
    ...
    for epoch in range(num_epochs):
        ...
        save_losses(train_losses, "../../outputs/wgan/train_losses.txt")
        ...
        if epoch % validation_rate == 0:
            ...
            save_losses(val_losses, "../../outputs/wgan/val_losses.txt")
