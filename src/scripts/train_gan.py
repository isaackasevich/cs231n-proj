...
#     def __init__(self, img_shape, channel_1=32, channel_2=64, channel_3=32):
#         super().__init__()
...
#         self.unconv1 = nn.ConvTranspose2d(channel_2, channel_3, 5, stride=2, padding=1)
#         self.batchNorm3 = nn.BatchNorm2d(channel_3)
#         self.unconv2 = nn.ConvTranspose2d(channel_3, 3, 3, stride=2, padding=0)
...
#     def forward(self, x):
#         x = self.maxPool(self.batchNorm1(self.relu(self.conv1(x))))
#         x = self.maxPool(self.batchNorm2(self.relu(self.conv2(x))))
#         x = self.batchNorm3(self.relu(self.unconv1(x)))
#         out_img = self.relu(self.unconv2(x))
        
#         return out_img

class Generator(nn.Module):
    def __init__(self, img_shape, channel_1=32, channel_2=16, channel_3=9, channel_4=5):
        super().__init__()
...
def train_batches(epoch, generator, discriminator, dataloader, train=True, save=True):
    losses = []
    #Loop through batch
...
def train_model(generator, discriminator, save=True):
    
    train_losses = []
    val_losses = []
    for epoch in range(num_epochs):
        train_losses += train_batches(epoch, generator, discriminator, train_dataloader)
        save_losses(train_losses, "../../outputs/gan/train_losses.txt")

        if epoch % validation_rate == 0:
            val_losses += train_batches(epoch, generator, discriminator, val_dataloader, train=False, save=False)
            save_losses(val_losses, "../../outputs/gan/val_losses.txt")
